...
def build_report(scan: Scan) -> bytes:
    ...
    overall_text = (
        f"<b>Overall status:</b> <font color='{overall_colour.hexval()}'>"
        f"{scan.overall_status.upper()}</font>"
    )
    elements.extend(
        [
            Paragraph(overall_text, styles["Normal"]),
            Spacer(1, cm),
            PageBreak(),
    ...
    for row, verdict in enumerate(scan.verdicts, start=1):
        table.setStyle(
            TableStyle(
                [("TEXTCOLOR", (2, row), (2, row), STATUS_COLOUR.get(verdict.status, colors.black))]
            )
        )
    rule_version = scan.verdicts[0].rule_version if scan.verdicts else "n/a"
    elements.extend(
        [
            table,
            Spacer(1, cm),
            Paragraph(
                f"<i>Rules applied: rules.yaml version {rule_version}</i>",
                styles["Normal"],
            ),
        ]
    )
