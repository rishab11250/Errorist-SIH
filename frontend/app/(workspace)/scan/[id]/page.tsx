...
export default function ScanResultPage() {
  ...
  useEffect(() => {
    ...
    getScan(scanId)
      ...
      .catch((reason) =>
        setError(reason instanceof Error ? reason.message : 'The scan could not be loaded.')
      );
  }, [scanId]);
    ...
    return (
      <div className="mx-auto max-w-2xl p-6">
        <div role="alert" className="rounded-xl border border-red-200 bg-red-50 p-5 text-red-900">
          ...
          <a
            href="/"
            className="mt-4 inline-flex min-h-11 items-center rounded-lg bg-red-800 px-4 py-2 font-semibold text-white"
          >
            Start a new scan
          </a>
        </div>
    ...
    <div className="mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-6 sm:py-10">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap gap-3">
          <a
            href="/"
            className="inline-flex min-h-11 items-center rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-800 hover:bg-slate-50 focus:outline-none focus:ring-4 focus:ring-blue-100"
          >
            New scan
          </a>
          <a
            ...
            className="inline-flex min-h-11 items-center rounded-lg bg-blue-700 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-200"
          >
            Download PDF
          </a>
        </div>
      ...
      <section
        ...
        className={`rounded-xl border p-5 ${qualityClasses[scan.quality.status]}`}
      >
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider">Image assessment</p>
            <h2 id="quality-heading" className="mt-1 text-lg font-bold">
              {qualityLabels[scan.quality.status]}
            </h2>
          </div>
          <p className="rounded-full bg-white/70 px-3 py-1 text-sm font-semibold">
            Quality score {Math.round(scan.quality.score * 100)}%
          </p>
        </div>
        {scan.quality.guidance.length > 0 ? (
          <ul className="mt-4 list-disc space-y-1 pl-5 text-sm leading-6">
            {scan.quality.guidance.map((item) => (
              <li key={item}>{item}</li>
            ))}
          </ul>
        ) : (
      ...
      <section
        ...
        className={`rounded-xl border p-5 ${overallClasses[scan.overallStatus]}`}
      >
        ...
        {scan.overallStatus === 'manual_review' && (
          <p className="mt-2 text-sm">
            The image does not provide enough reliable evidence for a final automated decision.
          </p>
        )}
      </section>
      ...
      <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(22rem,1fr)]">
        <section
          aria-label="Annotated package image"
          className="overflow-hidden rounded-xl border border-slate-200 bg-white p-3 shadow-sm lg:sticky lg:top-6"
        >
        ...
        <section aria-labelledby="checks-heading" className="space-y-3">
          <h2 id="checks-heading" className="text-xl font-bold text-slate-950">
            Declaration checks
          </h2>
          {scan.verdicts.map((verdict) => (
            <VerdictCard key={verdict.rule_id} verdict={verdict} />
          ))}
        </section>
      </div>
    </div>
