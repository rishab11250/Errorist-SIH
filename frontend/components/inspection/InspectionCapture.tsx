...
import { blobToBase64 } from '@/lib/offline-sync';
...
const DEFAULT_SECTIONS: SectionSlot[] = [
  {
    id: 'sec-1',
    label: '1. Front / Brand & Product Name',
    file: null,
    preview: null,
    dimensions: null,
    preScanQuality: null,
  ...
  {
    id: 'sec-2',
    label: '2. Nutrition & Ingredients Panel',
    file: null,
    preview: null,
    dimensions: null,
    preScanQuality: null,
  ...
  {
    id: 'sec-3',
    label: '3. Manufacturer, FSSAI & Barcode',
    file: null,
    preview: null,
    dimensions: null,
    preScanQuality: null,
  ...
  {
    id: 'sec-4',
    label: '4. MRP, Net Quantity & Dates Flap',
    file: null,
    preview: null,
    dimensions: null,
    preScanQuality: null,
...
async function persistSnapshot(
  ...
  const { scan_id } = await apiFetch<{ scan_id: number }>('/api/scan/sync', {
    method: 'POST',
    signal,
    body: JSON.stringify({
...
function inferredCategory(category: ScanContext['category'], words: OCRWord[]) {
  if (category !== 'unknown') return category;
  const text = words
    .filter((word) => word.confidence >= 0.45)
    .map((word) => word.text)
    .join(' ');
  return /\b(?:ingredients?|nutrition(?:al)?|fssai|protein|carbohydrate|allergen)\b/i.test(text)
    ? 'food'
    : 'unknown';
}

function scopeSinglePanelVerdicts(verdicts: Verdict[], mode: ScanContext['mode']): Verdict[] {
  if (mode !== 'retail_image') return verdicts;
  return verdicts.map((verdict) =>
    verdict.status === 'fail' && !verdict.evidence.trim()
      ? {
          ...verdict,
          status: 'manual_review',
          reasoning:
            'This declaration was not found on the photographed panel. Capture the remaining package panels before concluding it is absent.',
...
export function InspectionCapture({ onComplete }: Props) {
  ...
  const handleRotateImage = useCallback(async () => {
    ...
    try {
      ...
      if (rotatedBlob) {
        ...
        console.log(
          `[InspectionCapture] Rotated image 90° clockwise: ${rotCanvas.width}x${rotCanvas.height}px`
        );
      }
    } catch (err) {
  ...
  async function handleScan() {
    ...
    if (mode === 'ecommerce_listing' && !ecommerceConfirmed) {
      setError(
        'Confirm that the uploaded evidence is an e-commerce product-page screenshot. Physical package photos must use Retail package photo.'
      );
      setStage('error');
      return;
    }
    ...
    try {
      if (scanWorkflow === 'multi') {
        ...
        for (let i = 0; i < activeSections.length; i++) {
          ...
          sectionCaptures.push({
            id: sec.id,
            label: sec.label,
            file: ocr.evidenceFile ?? scanFile,
            ocr,
            quality,
          });
        }
        ...
        const resolvedCategory = inferredCategory(category, allWords);
        if (resolvedCategory !== category) setCategory(resolvedCategory);
        const scanContext: ScanContext = { mode, category: resolvedCategory, imported };
        ...
        try {
          ...
          throw new Error(
            'Could not save this scan on your device. Free storage space and retry; the scan has not been saved.'
          );
        }
        if (operation !== operationRef.current) return;
        ...
        if (isOffline) {
          setStage('saving');
          setStage('complete');
          onComplete({ ...compositeOCR, response: localResponse, scanContext });
          return;
        }

        let finalResponse = localResponse;
        try {
          finalResponse = await persistSnapshot(pendingRecord, localResponse, controller.signal);
          if (operation !== operationRef.current) return;
          try {
        ...
        setStage('saving');
        setStage('complete');
        onComplete({ ...compositeOCR, response: finalResponse, scanContext });
        return;
      }
      ...
      const rawFile = secondaryFile ? await createCompositeEvidenceFile(file, secondaryFile) : file;
      const scanFile = await downscaleImageFile(rawFile, 1600);
      const rules = loadDefaultRules();
      const initialScanContext: ScanContext = { mode, category, imported };
      const languages = ocrLanguage === 'multi' ? ['eng', 'hin'] : ['eng'];
      setStage('ocr');
      ...
      const resolvedCategory = inferredCategory(category, ocr.words);
      if (resolvedCategory !== category) setCategory(resolvedCategory);
      const scanContext: ScanContext = { mode, category: resolvedCategory, imported };
      const packageCheck = assessPackageContent(ocr.words);
      console.log(
        `[InspectionCapture] Package anchor check: isPackage=${packageCheck.isPackage}, score=${packageCheck.score}, anchors: [${packageCheck.anchorsFound.join(', ')}]`
      );
      if (!packageCheck.isPackage) {
        throw new Error(
          'Could not verify a readable package label. Include at least two clear declarations, such as MRP and Net Quantity, or manufacturer and ingredients. Rotate the photo or capture a closer label panel. No inspection has been saved.'
        );
      }
      setStage('analyzing');
      ...
      const localExtracted =
        resolvedCategory === category && ocr.extractedFields
          ? ocr.extractedFields
          : extractAll(ocr.words, imageMeta, scanContext, rules);
      const localVerdicts = scopeSinglePanelVerdicts(
        runEngine({ extracted: localExtracted, quality: localQuality }, rules, scanContext),
        mode
      );
      ...
      const pendingRecord: PendingScanRecord = {
        ...
        captured_at: new Date().toISOString(),
        rule_version: rules.version,
        image_blob: ocr.evidenceFile ?? scanFile,
        ocr_payload: ocr.words,
        scan_context: scanContext,
      ...
      if (isOffline) {
        setStage('saving');
        setStage('complete');
        onComplete({ ...ocr, response: localResponse, scanContext });
        return;
      }
      ...
      setStage('saving');
      setStage('complete');
      onComplete({ ...ocr, response: finalResponse, scanContext });
    } catch (reason) {
      if (operation !== operationRef.current) return;
      setStage('error');
      setError(
      ...
      <div className="relative mx-auto max-w-5xl space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-[#E7E2D8] pb-5">
          <div>
            ...
            <p className="text-sm text-ink-muted mt-0.5 max-w-2xl">
              Photograph physical packaged goods or import e-commerce evidence to extract statutory
              declarations and run instant rule verification.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2.5 sm:gap-3">
            <div className="px-3 py-1.5 sm:px-3.5 sm:py-2 rounded-lg bg-surface-dim border border-[#E2DDD3] text-[11px] sm:text-xs font-mono">
              ...
              <span className="font-bold text-ink">Rules 2011 (LMPC)</span>
            </div>
            <button
              type="button"
              onClick={() => {
            ...
            <div className="bg-surface-card p-4 rounded-xl border border-[#E8E2D6] shadow-kinetic-sm">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 p-1 bg-surface-dim rounded-lg border border-[#E0D9CD]">
                  ...
                  <span className="flex items-center gap-2">
                    <span
                      className={cn(
                        ...
                        scanWorkflow === 'single'
                          ? 'bg-terracotta'
                          : 'bg-transparent border border-ink-muted'
                      )}
                    ></span>
                    <span>Standard Scan</span>
                  </span>
                  <span className="text-[10px] font-mono text-ink-muted bg-[#F2EDE4] px-1.5 py-0.5 rounded">
                    Single/Dual
                  </span>
                </button>
                <button
                ...
                >
                  ...
                  <span className="text-[10px] font-mono text-terracotta font-bold bg-terracotta-light px-1.5 py-0.5 rounded">
                    Bulk/Tall
                  </span>
                </button>
              </div>
          ...
          <div className="lg:col-span-6 space-y-4">
            ...
            >
              <span
                ...
                className="block text-xs font-mono uppercase font-bold text-ink-muted mb-2 tracking-wider"
              >
                3. Evidence Source Channel
              </span>
            ...
            {mode === 'ecommerce_listing' ? (
              <label className="mt-3 flex items-start gap-2 rounded-lg border border-amber/30 bg-amber-bg p-3 text-xs text-ink">
                <input
                  type="checkbox"
                  checked={ecommerceConfirmed}
                  onChange={(event) => setEcommerceConfirmed(event.target.checked)}
                  disabled={busy}
                  className="mt-0.5"
                />
                I confirm this is a screenshot of a product listing page, not a camera photo of a
                physical packet.
              </label>
            ) : null}
            ...
            <div className="bg-surface-card p-4 rounded-xl border border-[#E8E2D6] shadow-kinetic-sm">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block text-[11px] font-semibold text-ink mb-1 font-heading">
                    Product Category
                  </label>
                ...
                <div>
                  ...
                  <div className="relative">
                    <select
                      ...
                      onChange={(e) =>
                        setImported(
                          e.target.value === 'unknown' ? null : e.target.value === 'imported'
                        )
                      }
                      disabled={busy}
                      className="w-full text-xs font-medium bg-surface-dim border border-[#D5CFC4] rounded-lg px-3 py-2.5 appearance-none focus:outline-none focus:ring-2 focus:ring-terracotta/40 focus:border-terracotta cursor-pointer"
          ...
          <div className="space-y-5 bg-surface-card p-5 rounded-2xl border border-[#E0D9CD] shadow-kinetic-sm">
            <div className="rounded-lg border border-terracotta/30 bg-terracotta-light/30 p-4 text-sm text-ink space-y-1">
              <div className="flex items-center gap-2 font-semibold font-heading text-terracotta">
                <Layers className="size-4" /> Multi-Section Bulk Scanner for Tall / Detailed
                Packaging
              </div>
              <p className="text-ink-muted text-xs leading-relaxed font-sans">
                Capture small close-up sections of long or tall packages (e.g. noodles, rolls) at
                native resolution. The engine verifies all sections belong to the same product,
                averages quality scores, and synthesizes a complete statutory report.
              </p>
            </div>
                ...
                {mode === 'retail_image' && !file && captureMethod === 'camera' ? (
                  <div className="space-y-3">
                    ...
                    <div className="text-center">
                      ...
                      >
                        <ImageUp className="size-3 text-terracotta" /> Prefer uploading an existing
                        photo from gallery? Switch to file upload
                      </button>
                    </div>
                  </div>
                ) : (
                  <>
                    <div
                      ...
                      className={cn(
                        ...
                        isDragging
                          ? 'border-terracotta ring-4 ring-terracotta/20 scale-[1.01]'
                          : 'border-[#2B2925]'
                      )}
                    >
                      {/* Grid overlay lines */}
                        ...
                        <>
                          ...
                          <div className="absolute inset-3 sm:inset-6 md:inset-10 border-2 border-dashed border-terracotta/60 rounded-lg pointer-events-none flex flex-col justify-between p-2.5 sm:p-3">
                            ...
                            <div className="self-center flex flex-col items-center my-auto pb-12 sm:pb-0">
                              ...
                              <p className="text-[11px] sm:text-xs font-mono text-white/60 mt-0.5">
                      ...
                      <div className="space-y-3">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {mode === 'retail_image' && (
                              <button
                                ...
                                className="inline-flex items-center gap-1.5 rounded-lg border border-[#D5CFC4] bg-white px-3.5 py-2 text-xs font-heading font-semibold text-ink hover:bg-surface-dim transition shadow-kinetic-sm"
                              >
                                <Camera className="size-3.5 text-terracotta" /> Retake with Guided
                                Camera
                              </button>
                            )}
                          </div>
                        ...
                        <div className="bg-surface-dim rounded-xl p-3 border border-[#E0D9CD] flex flex-wrap items-center justify-between gap-3 text-xs">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-forest-light text-forest font-mono font-semibold text-[11px] border border-forest/30">
                              <span className="w-1.5 h-1.5 rounded-full bg-forest"></span> Lighting:
                              480 LUX
                            </span>
                            {preScanQuality && (
                              <span
              ...
              <div className="lg:col-span-4 flex flex-col justify-between space-y-4">
                <div className="bg-surface-dim/60 rounded-xl p-4 border border-[#E2DDD3] space-y-3">
                  ...
                  <p className="text-xs text-ink-muted leading-snug">
                    Attach close-up of sticker overlays, batch numbers, or bottom seal if printed
                    separately.
                  </p>
                    ...
                    <div className="p-3 bg-white rounded-lg border border-[#D5CFC4] flex items-center justify-between shadow-kinetic-sm">
                      <div className="flex items-center space-x-3 overflow-hidden">
                        ...
                        <div className="overflow-hidden">
                          <p className="text-xs font-bold font-heading text-ink truncate">
                            {secondaryFile.name}
                          </p>
        ...
        <button
          ...
          disabled={
            cropping ||
            busy ||
            (scanWorkflow === 'single' ? !file : sections.filter((s) => s.file !== null).length < 2)
          }
        ...
        >
          ...
          {stage === 'error'
            ...
            : scanWorkflow === 'multi'
              ...
              : mode === 'ecommerce_listing'
                ? 'Inspect e-commerce listing screenshot'
                : 'Inspect photographed package panel'}
        </button>
      </div>
    </div>
  );
